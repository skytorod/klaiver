	...
	<section id="container">
		<div id="msg_box2">
			<ul id="follow_list2">
               ...
               <input type="hidden" id="email" value="${soloViewfollower.followid}">
					...
					<li><span class="profile_img" style='cursor: pointer;'
						...
						<div>
							<div>
								...
								${soloViewfollower.first}${soloViewfollower.last}<br>${soloViewfollower.company}</span>
								<c:if test="${soloViewfollower.flag eq true && sessionScope.login ne soloViewfollower.myid}">
									<span class="follow_btn"><a
										href="../follow/delete?userid=${sessionScope.login}&followid=${soloViewfollower.myid}">Following</a></span>
								</c:if>
								<c:if test="${soloViewfollower.flag eq true && sessionScope.login eq soloViewfollower.myid}">
									<span class="follow_btn"></span>
								</c:if>
								<c:if test="${soloViewfollower.flag eq false && sessionScope.login ne soloViewfollower.myid}">
									<span class="follow_btn"><a
										href="../follow/insertFollow?userid=${sessionScope.login}&followid=${soloViewfollower.myid}">Follow</a></span>
								</c:if>
								<c:if test="${soloViewfollower.flag eq false && sessionScope.login eq soloViewfollower.myid}">
									<span class="follow_btn"></span>
								</c:if>
							</div>
						</div></li>
						...
						success : function(data) {
							...
							if (data != "") {
								$(data).each(function() {
									...
									var id = $("#email").val();
									var sessionid = $("#sessionid").val();
									if(this.flag == true && sessionid != myid){
										str +="<ul id='follow_list2'>" 
											...
											+"<a href='../follow/delete?userid="+sessionid+"&followid="+myid+"'>Following</a></span>"
											+"</div>"
											+"</div></li></ul>";
									}else if(this.flag == false  && sessionid != myid){
										str +="<ul id='follow_list2'>" 
											+"<div class='scrolling' data-fid='"+fid+"'>"
											+"<input type='hidden' id='email' value='"+id+"'></div>"
											+"<li><span class='profile_img' style='cursor: pointer;' onclick=newpage('"+myid+"');>"
											...
											+first+last+"<br>"+company+"</span>"
											+"<span class='follow_btn'>"
											+"<a href='../follow/insertFollow?userid="+sessionid+"&followid="+myid+"'>Follow</a></span>"
											+"</div>"
											+"</div></li></ul>";
									}else if(this.flag == false && sessionid == myid){
										str +="<ul id='follow_list2'>" 
