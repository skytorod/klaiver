			...
			<ul id="follow_list2">
				<c:forEach items="${myCompanyfolloer}" var="myCompanyfolloer">
					<div class="scrolling" data-fid="${myCompanyfolloer.fcid}">
						<input type="hidden" id="email" value="${sessionScope.login}">
					</div>
					<li><span class="profile_img" style='cursor: pointer;'
						onclick="newpage('${myCompanyfolloer.fcmyid}');"><img
							src="../resources/img/${myCompanyfolloer.profimg}" /></span>

						<div>
							<div>
								<span class="user" style='cursor: pointer;'
									onclick="newpage('${myCompanyfolloer.fcmyid}');">
									${myCompanyfolloer.first}${myCompanyfolloer.last}<br>${myCompanyfolloer.company}</span>
								<c:if test="${myCompanyfolloer.flag eq true && sessionScope.login ne myCompanyfolloer.fcmyid}">
									<span class="follow_btn"><a
										href="../follow/delete?userid=${sessionScope.login}&followid=${myCompanyfolloer.fcmyid}">Following</a></span>
								</c:if>
								<c:if test="${myCompanyfolloer.flag eq true && sessionScope.login eq myCompanyfolloer.fcmyid}">
									<span class="follow_btn"></span>
								</c:if>
								<c:if test="${myCompanyfolloer.flag eq false && sessionScope.login ne myCompanyfolloer.fcmyid}">
									<span class="follow_btn"><a
										href="../follow/insertFollow?userid=${sessionScope.login}&followid=${myCompanyfolloer.fcmyid}">Follow</a></span>
								</c:if>
								<c:if test="${myCompanyfolloer.flag eq false && sessionScope.login eq myCompanyfolloer.fcmyid}">
									<span class="follow_btn"></span>
								</c:if>
							</div>
						</div>
	...
	$(document).ready(function() {
			...
			$(window).scroll(function() {
				...
				if (scrollpercent >= 0.99&& !scrollEventFlag) {
					...
					$.ajax({
						type : 'POST',
						url : '../member/soloComfollowerScroll',
						headers : {
								...
								$(data).each(function() {
									...
									if(this.flag == true && id != fcmyid){
										str +="<ul id='follow_list2'>" 
											...
											+"<a href='../follow/deleteCom?userid="+id+"&followid="+fcmyid+"'>Following</a></span>"
											+"</div>"
											+"</div></li></ul>";
									}else if(this.flag == false && id != fcmyid){
										str +="<ul id='follow_list2'>" 
											+"<div class='scrolling' data-fid='"+fcid+"'>"
											+"<input type='hidden' id='email' value='"+id+"'></div>"
											+"<li><span class='profile_img' style='cursor: pointer;' onclick=newpage('"+fcmyid+"');>"
											...
											+first+last+"<br>"+company+"</span>"
											+"<span class='follow_btn'>"
											+"<a href='../follow/insertComFollow?userid="+id+"&followid="+fcmyid+"'>Follow</a></span>"
											+"</div>"
											+"</div></li></ul>";
									}else if(this.flag == false && id == fcmyid){
										str +="<ul id='follow_list2'>" 
