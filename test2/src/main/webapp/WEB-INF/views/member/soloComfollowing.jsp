					...
					<li><span class="profile_img" style='cursor: pointer;'
						...
						<div>
							<div>
								...
								<c:if test="${myCompanyfolloer.flag eq true}">
								...
								<c:if test="${myCompanyfolloer.flag eq false}">
									<span class="follow_btn"><a
	...
	$(document).ready(function() {
			...
			$(window).scroll(function() {
				...
				if (scrollpercent >= 0.99&& !scrollEventFlag) {
					...
					$.ajax({
						type : 'POST',
						url : '../member/soloComfollowingScroll',
						headers : {
						...
						success : function(data) {
							...
							if (data != "") {
								$(data).each(function() {
									...
									var flag = this.flag;
									var id = $("#email").val();
									if(this.flag == true && id != fcfollowid){
										str +="<ul id='follow_list2'>" 
											...
											+"<a href='../follow/deleteCom?userid="+id+"&followid="+fcfollowid+"'>Following</a></span>"
											+"</div>"
											+"</div></li></ul>";
										...
										str +="<ul id='follow_list2'>" 
											...
											+first+last+"<br>"+company+"</span>"
											+"<span class='follow_btn'>"
											+"<a href='../follow/insertComFollow?userid="+id+"&followid="+fcfollowid+"'>Follow</a></span>"
											+"</div>"
											+"</div></li></ul>";
									}else if(this.flag == true && id != fcfollowid){
										str +="<ul id='follow_list2'>" 
											+"<div class='scrolling' data-fid='"+fcid+"'>"
											+"<input type='hidden' id='email' value='"+id+"'></div>"
											+"<li><span class='profile_img' style='cursor: pointer;' onclick=newpage('"+fcfollowid+"');>"
