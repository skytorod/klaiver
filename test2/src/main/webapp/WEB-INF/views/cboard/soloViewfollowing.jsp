					...
					<li><span class="profile_img" style='cursor: pointer;'
						...
						<div>
							<div>
								...
								${soloViewfollowing.username}<br>${soloViewfollowing.email}</span>
								<c:if test="${soloViewfollowing.flag eq true}">
	...
	$(document).ready(function() {
			...
			$(window).scroll(function() {
				...
				if (scrollpercent >= 0.99&& !scrollEventFlag) {
					...
					$.ajax({
						type : 'POST',
						url : '../member/soloViewfollowingScroll',
						headers : {
								...
								$(data).each(function() {
									...
									if(this.flag == true && id != followid){
										str +="<ul id='follow_list2'>" 
											...
											+"<a href='../follow/delete?userid="+id+"&followid="+followid+"'>Following</a></span>"
											+"</div>"
											+"</div></li></ul>";
										...
										str +="<ul id='follow_list2'>" 
											...
											+first+last+"<br>"+company+"</span>"
											+"<span class='follow_btn'>"
											+"<a href='../follow/insertFollow?userid="+id+"&followid="+followid+"'>Follow</a></span>"
											+"</div>"
											+"</div></li></ul>";
									}else if(this.flag == true && id == followid){
										str +="<ul id='follow_list2'>" 
											+"<div class='scrolling' data-fid='"+fid+"'>"
											+"<input type='hidden' id='email' value='"+id+"'></div>"
											+"<li><span class='profile_img' style='cursor: pointer;' onclick=newpage('"+followid+"');>"
